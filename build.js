...
function prepare(site, config) {
  ...
  const content = new Content[storage](config);
  const apiDir = 'build';
 ...
 * Exports media files.
 */
function mediaExport(config, callback) {
  const buildDir = path.join(config.get('buildDir'), 'media');
 ...
 * Exports routes.
 */
function routesExport(config, callback) {
  const content = prepare(config);
  const routeCollections = content.schema.getConfigItem('routeCollections');
  const file = path.join(config.get('buildDir'), 'api/v1/routes.json');
 ...
 */
function docsExport(config, env, callback) {
  const content = prepare(config);
  ...
  content.findAll(true, (err, results) => {
    Async.each(content.collections, (collection, done) => {
      ...
      const file = path.join(config.get('buildDir'), `${collection}.json`);
 ...
 */
function docExport(config, collection, interraId, env, callback) {
  const content = prepare(config);
  const siteInfo = new Site(config);
  const re = new RegExp('\\[interraUrl\\]', 'g');
...
function schemaExport(site, config, callback) {
  ...
  const apiDir = path.join(config.get('buildDir'), site);
